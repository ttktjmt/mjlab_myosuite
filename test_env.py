...
def test_task_registration():
    ...
    try:
        ...
        if task_id in _TASK_REGISTRY:
            ...
            print(f"  - Runner class: {info.runner_cls.__name__ if info.runner_cls else 'None'}")
            return True
        else:
...
def test_rotation_utilities():
    ...
    try:
        ...
        print(f"✓ Quaternion distance: {dist.item():.3f} radians (~{dist.item()*180/3.14159:.1f} degrees)")
...
def main():
    ...
    if passed == total:
        ...
        print("\nNext steps:")
        print("  1. Run with zero agent: uv run play Myosuite-Manipulation-DieReorient-Myohand --agent zero")
        print("  2. Run with random agent: uv run play Myosuite-Manipulation-DieReorient-Myohand --agent random")
        print("  3. Start training: uv run train Myosuite-Manipulation-DieReorient-Myohand --env.scene.num-envs 512")
        return 0
    else:
