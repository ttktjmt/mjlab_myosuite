...
def register_myosuite_envs(prefix: str = "Mjlab-MyoSuite"):
  ...
  for env_id in myosuite_envs:
    ...
    gym.register(
      id=mjlab_env_id,
      entry_point="mjlab.tasks.myosuite.env_factory:make_myosuite_env",
      disable_env_checker=True,
      kwargs={
        "myosuite_env_id": env_id,
        "env_cfg_entry_point": "mjlab.tasks.myosuite.config:MyoSuiteEnvCfg",
        "rl_cfg_entry_point": "mjlab.tasks.myosuite.config:get_default_myosuite_rl_cfg",
      },
    )
...
def list_myosuite_envs() -> list[str]:
  """List all available MyoSuite environment IDs.

  Returns:
    List of MyoSuite environment IDs
  """
  try:
    import myosuite.utils.gym  # noqa: F401
  except ImportError:
    return []
